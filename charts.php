...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    // Get the id from the query string and escape it
    $id = mysqli_real_escape_string($conn, $_GET['id']);

    // Query to fetch website details based on the id
    $query = "SELECT * FROM website_info WHERE id = '$id'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Redirect or handle the case where the website is not found
        header("Location: index.php"); // Or show an error message
        exit();
    }
} else {
                    ...
                    <div class="container mt-5">
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h3 class="m-0 font-weight-bold text-primary text-center">Edit Website</h3>
                            </div>
                            <div class="card-body">
                                <form action="update_website.php" method="POST">
                                    <!-- Hidden field to store website ID -->
                                    <input type="hidden" name="id" value="<?php echo htmlspecialchars($website['id']); ?>">

                                    <!-- Website Name field -->
                                    <div class="form-group">
                                        <label for="website_name">Website Name</label>
                                        <input type="text" class="form-control" id="website_name" name="website_name" value="<?php echo htmlspecialchars($website['website_name']); ?>" required>
                                    </div>

                                    <!-- Domain Name field -->
                                    <div class="form-group">
                                        <label for="domain_name">Domain Name</label>
                                        <input type="text" class="form-control" id="domain_name" name="domain_name" value="<?php echo htmlspecialchars($website['domain_name']); ?>" required>
                                    </div>

                                    <!-- Hosting Company field -->
                                    <div class="form-group">
                                        <label for="hosting_company">Hosting Company</label>
                                        <input type="text" class="form-control" id="hosting_company" name="hosting_company" value="<?php echo htmlspecialchars($website['hosting_company']); ?>" required>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="country" name="country" class="form-control" required>
                                            ...
                                            if ($result->num_rows > 0) {
                                                // Output each country as an option
                                                while ($row = $result->fetch_assoc()) {
                                                    // Set selected attribute if this is the currently selected country
                                                    $selected = ($row['name'] == $website['country']) ? 'selected' : '';
